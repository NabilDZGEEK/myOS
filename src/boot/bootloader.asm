...
;---------------------------------------------------------
%include "src/boot/AFFICHAGE.INC"

start:

; initialisation des segments 
    mov ax, 0x07C0
    mov ds, ax  ;ds <- 0x07C0 : adresse physique =0x7C00+[org]=0x7C00 segment de données
    mov es, ax  ;ds <- 0x07C0;
...
%define KSIZE 50     ; nombre de secteurs à charger
    ...
    mov dh, 0 ;tete N°0
    int 0x13 ;intéruption de chargement
    pop es
...
; passage en mode protégé
    ...
    lgdt [gdtptr]    ; le registre gdtr <- adresse de la gdt
    mov eax, cr0
    or  ax, 1
    ...
    jmp next
next:
    mov ax, 0x10        ; réinitialisation du segment de données  (pointe vers gdt_ds)
    mov ds, ax
    mov fs, ax
    mov gs, ax
    mov es, ax
